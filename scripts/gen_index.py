...
def main():
  ...
  parser = argparse.ArgumentParser(
    description="""
    This script is used to generate the index.{js|ts} files of the current directory
    """
  )
  parser.add_argument(
    "-r",
  ...
  parser.add_argument(
    "directory",
    help=f"""
    The directory to generate the index.{{js|ts}} file in. (default: {directory})
    """,
    nargs="?",
    default=directory
  )
  parser.add_argument(
    "--ext",
    help="The file extension to use for the index.{js|ts} file. (default: \".ts\")",
    choices=[".ts", ".js"],
    default=".ts"
  )
...
def gen_index(dir: str, args):
  chdir()
  if args.verbose:
    dir_name = args.directory.replace(os.path.dirname(args.directory) + os.sep, "")
    sub_dir = dir.replace(args.directory + os.sep, "").replace(args.directory, "")
  ...
  try:
    ...
    print(f"Failed to list directory contents of:\n{dir}")
    sys.exit(1)
...
def chdir(path: str = ""):
  ...
  try:
    ...
    print(f"Failed to change directory to:\n{path}")
    sys.exit(1)
  return path
